...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('CaptionTV') and o.name = 'FK_CAPTIONT_CAPTION_CAPTIONG')
alter table CaptionTV
   drop constraint FK_CAPTIONT_CAPTION_CAPTIONG
go
...
if exists (select 1
           ...
           where  id    = object_id('CaptionTV')
            ...
            and   indid > 0
            and   indid < 255)
   drop index CaptionTV.Caption_FK
go
...
if exists (select 1
           ...
           where  id    = object_id('OriginalDisk')
            and   name  = 'Is_OriginalDisk_FK'
            and   indid > 0
            and   indid < 255)
   drop index OriginalDisk.Is_OriginalDisk_FK
go
...
if exists (select 1
           ...
           where  id    = object_id('SuppressBD')
            ...
            and   indid > 0
            and   indid < 255)
   drop index SuppressBD.Is_BD_FK
go
...
create table AnimeResource (
   ...
   constraint PK_ANIMERESOURCE primary key nonclustered (ResourceType)
)
go
...
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'AnimeResource', 'column', 'ResourceType'
...
create table CaptionTV (
   ...
   ResourceType         varchar(1)           null,
   CaptionGroup_Name    varchar(1)           null,
   constraint PK_CAPTIONTV primary key nonclustered (TV_Name)
)
go
...
if exists(select 1 from sys.extended_properties p where
      p.major_id = object_id('CaptionTV')
  and p.minor_id = (select c.column_id from sys.columns c where c.object_id = p.major_id and c.name = 'TV_Name')
)
begin
   declare @CurrentUser sysname
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'CaptionTV', 'column', 'TV_Name'
...
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'CaptionTV', 'column', 'ResourceType'
...
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'CaptionTV', 'column', 'CaptionGroup_Name'
...
/* Index: Is_TV_FK                                              */
/*==============================================================*/
create index Is_TV_FK on CaptionTV (
ResourceType ASC
)
go
...
/* Index: Caption_FK                                            */
/*==============================================================*/
create index Caption_FK on CaptionTV (
CaptionGroup_Name ASC
)
go
...
create table OriginalDisk (
   ...
   ResourceType         varchar(1)           null,
   constraint PK_ORIGINALDISK primary key nonclustered (Original_OrderNum)
)
go
...
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'OriginalDisk', 'column', 'ResourceType'
...
/*==============================================================*/
/* Index: Is_OriginalDisk_FK                                    */
/*==============================================================*/
create index Is_OriginalDisk_FK on OriginalDisk (
ResourceType ASC
)
go
...
create table SuppressBD (
   ...
   constraint PK_SUPPRESSBD primary key nonclustered (BD_Name)
)
go
...
if exists(select 1 from sys.extended_properties p where
      p.major_id = object_id('SuppressBD')
  and p.minor_id = (select c.column_id from sys.columns c where c.object_id = p.major_id and c.name = 'BD_Name')
)
begin
   declare @CurrentUser sysname
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'SuppressBD', 'column', 'BD_Name'
...
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'SuppressBD', 'column', 'Original_OrderNum'
...
select @CurrentUser = user_name()
execute sp_dropextendedproperty 'MS_Description', 
   'user', @CurrentUser, 'table', 'SuppressBD', 'column', 'ResourceType'
...
/* Index: Is_BD_FK                                              */
/*==============================================================*/
create index Is_BD_FK on SuppressBD (
ResourceType ASC
)
go
...
alter table CaptionTV
   add constraint FK_CAPTIONT_CAPTION_CAPTIONG foreign key (CaptionGroup_Name)
      references CaptionGroup (CaptionGroup_Name)
go

alter table CaptionTV
   add constraint FK_CAPTIONT_IS_TV_ANIMERES foreign key (ResourceType)
      references AnimeResource (ResourceType)
go

alter table OriginalDisk
   add constraint FK_ORIGINAL_IS_ORIGIN_ANIMERES foreign key (ResourceType)
      references AnimeResource (ResourceType)
go

alter table SuppressBD
   add constraint FK_SUPPRESS_IS_BD_ANIMERES foreign key (ResourceType)
      references AnimeResource (ResourceType)
go

alter table SuppressBD
   add constraint FK_SUPPRESS_ORIGINALD_ORIGINAL foreign key (Original_OrderNum)
